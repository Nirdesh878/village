    ...
    <div class="page-header">
        <div class="row align-items-end">
            <div class="col-lg-8">
                ...
                @php $session_data = Session::get('shg_session'); @endphp
        ...
        <div class="card"> 
          <div class="card-block pt-2 pb-2">
              <div class="mT-30"> 
                  ...
                  <form class="" method="post" action="{{ url('getLatLongSHG')}}">
                      @csrf
                      <div class="row"> 
                        ...
                        <div class="col-md-2 mb-3">
                          ...
                          <select class="form-control" name="country" id="country">
                              ...
                              @foreach ($countries as $row)
                                  <option value="{{ $row->id }}"
                                    {{(!empty(Session::get('shg_session')['country'])&& ((Session::get('shg_session')['country']) == $row->id ) ? "selected":"")}}>
                                      {{ $row->name }}</option>
                              @endforeach
                          </select>
                        </div>
                          ...
                          <select class="form-control agency_id" name="agency_id" id="agency_id">
                              ...
                              @foreach ($agency as $row)
                                  <option value="{{ $row->agency_id }}"
                                      {{ !empty(Session::get('shg_session')['agency_id']) && Session::get('shg_session')['agency_id'] == $row->agency_id ? 'selected' : '' }}>
                        ...
                        {{-- <div class="col-md-2 mb-3">
                          ...
                          <select class="form-control shg_id" name="shg_id" id="shg_id">
                              <option value="">--Select--</option>
                          </select>
                        </div> --}}
...
<script type="text/javascript" src="{{asset('assets\leaflet\leaflet.measurecontrol.js')}}"></script> 
<script type="text/javascript" src="{{asset('assets\leaflet\leaflet.zoomhome.min.js')}}"></script>
...
<script>
  $(document).ready(function(){
    ...
    // $('#agency_id').on('change', get_shg_list_id);
            ...
            if (agency_id > 0) {
                $.ajax({
                    ...
                    success: function(data) {
                        if (data != '') {
                            $('#shg_id').html(data);
                            @php echo !empty($session_data["shg_id"]) && $session_data["shg_id"]>0 ? "$('#shg_id').val('".$session_data["shg_id"]."');" : "" @endphp
                            $('#shg_id').trigger("change");
                        }
                    }
                });
            }
...
<script> 
 ...
 var childCount = cluster.getChildCount();
  ...
  var constant = $.map( get_data, function( e ) {
    ...
    markers.addLayer(L.marker([e.latitude,e.longitude],{icon : greenIcon}).addTo(markers).bindPopup("<div class='popup_div'><div class='row'><table class='table table-striped'>"+"<tr><td><b>SHG</b></td><td>"+e.shgName+"</td></tr>"+"<tr><td><b>Latitude</b></td><td>"+e.latitude+"</td></tr>"+"<tr><td><b>Longitude</b></td><td>"+e.longitude+"</td></tr>"+"</td></tr>"+"<tr><td><b>Location Name</b></td><td>"+e.location_name+"</td></tr>"+"<tr><td><b>Agency</b></td><td>"+e.agency_name+"</td></tr>"+ "<tr><td><b>Date</b></td><td>" + e.lat_long_date_time + "</td></tr>" + "</table></div></div>"));
  }); 
  markers.addTo(map); 
