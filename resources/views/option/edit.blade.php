                ...
                <div class="col-lg-8">
                    <div class="page-header-breadcrumb">
                        <ul class="breadcrumb-title">
                            ...
                            <li class="breadcrumb-item">Update Option</li>
                        </ul>
                    </div>
                ...
                <div class="col-sm-12">
                    ...
                    <div class="card">
                        <div class="card-block">
                            <div class="mT-30">
                                <form class="container" method="POST" action="{{ route('option.update',[$option->mst_category_id,$option->mst_sub_category_id,$option->mst_ques_id]) }}">
                                    @method('PATCH')
                                    @csrf
                                    ...
                                    <div class="row">
                                         ...
                                         <div class="row">
                                            ...
                                            <label for="validationCustom02">Option<span class="red">*</span></label>
                                        ...
                                        <div class="col-md-12 mb-3">
                                            ...
                                            <input type="text" class="form-control" name="mst_description" value="{{( !empty($option->mst_description) ? $option->mst_description : '')}}" id="mst_description" required>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row">
                                       ...
                                       <tbody>   
                                       @php 
                                       $mst_id1=explode(',',$option->mst_id1);
                                       $mst_point1=explode(',',$option->mst_point1);
                                       $mst_status1=explode(',',$option->mst_status1);
                                       $mst_ans_name1=explode(',',$option->mst_ans_name1);
                                        @endphp  
                                    @php for($i=0;$i<(count($mst_id1));$i++){ @endphp
                                    <tr>
                                    <td><span class="sn">{{$i+1}}</span><input type="hidden" name="mst_id[]" value="{{$mst_id1[$i]}}"></td>
                                    <td><input type="text" name="mst_ans_name[]" class="mst_ans_name form-control" value="{{$mst_ans_name1[$i]}}"></td>
                                    <td><input type="text" name="mst_point[]" class="mst_point form-control" value="{{$mst_point1[$i]!=0 ? $mst_point1[$i]: ''}}"></td>
                                    <td><select name="mst_status[]" class="form-control mst_status"><option value="A" @if($mst_status1[$i]=='A') selected="selected" @endif >Active</option>
                                    <option value="I" @if($mst_status1[$i]=='I') selected="selected" @endif>Inactive</option></select></td>
                                    <td>
                ...
                success: function (data) {
                        ...
                        if (data != '') {
                            $('#mst_sub_category_id').html(data);
                             $('#mst_sub_category_id').val("{{( !empty($option->mst_sub_category_id) ? $option->mst_sub_category_id : '')}}");
                              $('#mst_sub_category_id').trigger('change');
                        }
                    }
                });
            });
             $('#mst_sub_category_id').on('change',function(event){
                ...
                success: function (data) {
                        ...
                        if (data != '') {
                            ...
                            $('#mst_ques_id').val("{{( !empty($option->mst_ques_id) ? $option->mst_ques_id : '')}}");
                        }
                    }
                });
            });
...
function add_option(elem) {
    ...
    $('#mst_category_id').val("{{( !empty($option->mst_category_id) ? $option->mst_category_id : '')}}");
    $('#mst_category_id').trigger('change');
    $('.mst_status').trigger('change');
});
