...
@section('content')
                ...
                <div class="col-lg-8">
                    <div class="page-header-breadcrumb">
                        <ul class="breadcrumb-title">
                            ...
                            <li class="breadcrumb-item">Update Assign Task (Family)</li>
                        </ul>
                    </div>
                    <div style="clear: both"></div>
                    <div class="page-header-title">
                        <div class="d-inline">
                            <h4>Update Assign Task (Family)</h4>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-sm-12">
                    ...
                    <div class="card">
                        <div class="card-block">
                            <div class="mT-30">
                                <form class="container" method="POST" action="{{ route('preanalytics.update', $preanalytics[0]->id) }}" autocomplete="off">
                                    @method('PATCH')
                                    @csrf
                                    <input type="hidden" name="id" value="{{ ( !empty($preanalytics[0]->id) ? $preanalytics[0]->id : '') }}">
                                    <div class="row">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <select class="form-control task" name="task" id="task" required>
                                                <option value="">--Select--</option>
                                                <option value="A" <?php echo $preanalytics[0]->task == 'A' ? 'selected' :'' ?> >Analysis</option>
                                                <option value="R" <?php echo $preanalytics[0]->task == 'R' ? 'selected' :'' ?> >Rating</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="row" id="task_show">
                                        <div class="col-md-6 mb-3">
                                            <select class="form-control task_a1" name="task_a1" id="task_a1">
                                                <option value="">--Select--</option>
                                                <option value="P1" <?php echo $preanalytics[0]->task_a1 == 'P1' ? 'selected' :'' ?> >Part-1</option>
                                                <option value="P2" <?php echo $preanalytics[0]->task_a1 == 'P2' ? 'selected' :'' ?> >Part-2</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="row">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <input type="text" class="form-control" name="remark"  value="{{ $preanalytics[0]->remark }}" autocomplete="off" required>
                                        </div>
                                    </div>
                                    <div class="row">
    ...
    <script>
            ...
            if (agency_id != '') {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data != '') {
                            $('#shg_uin').html(data);
                            $('#shg_uin').val("{{$preanalytics[0]->shg_id}}");
            ...
            if (agency_id != '') {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data != '') {
                            $('#user_id').html(data);
                            $('#user_id').val("{{$preanalytics[0]->user_id }}");
        ...
        function get_family_list_task() {
            var obj = $(this);
            var agency_id = obj.val();
            var id = {{ ( !empty($preanalytics[0]->assignment_id) ? $preanalytics[0]->assignment_id : '0') }};
            if (agency_id != '') {
                $.ajax({
                    type: 'GET',
                    url: '/get_family_list_task',
                    data: '_token = <?php echo csrf_token() ?>&agency_id=' + agency_id + '&id=' + id,
                    success: function (data) {
                        if (data != '') {
                            $('#assignment_id').html(data);
                            $('#assignment_id').val("{{$preanalytics[0]->family_id }}");
                        }
                    }
                });
            }
        ...
        $(document).ready(function () {
            ...
            $('#agency_id').val("{{( !empty($preanalytics[0]->agency_id) ? $preanalytics[0]->agency_id : '')}}");
