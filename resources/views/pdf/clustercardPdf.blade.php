...
<head>
    ...
    <title>Cluster Development Card</title>
</head>
<style>
    .table1 {
        width: 100%;
...
<body>
    ...
    <div class="d-inline">
        <h3
            style="font-family: Arial, Helvetica, sans-serif;text-align:center;font-size:30px;color:rgba(7, 83, 146, 0.726);position: relative;top:-140px;">
            CLUSTER INSTITUTIONAL DEVELOPMENT CARD
        </h3>
    </div>

    <table class="table table-bordered table-stripped table1 " cellspacing="0" style="margin-top: -200px;">

        <tbody>
            <tr>
                <th width="17%" style="text-align: left">Cluster Name:</th>
                <td width="16%" style="text-align: left">{{ $profile[0]->name_of_cluster }}</td>
                <td width="16%" style="text-align: left">UIN:</td>
                <td width="16%" style="text-align: left">{{ $cluster->uin}}</td>
                <th width="16%" style="text-align: left">DATE:</th>
                <td width="26%" style="text-align: left">{{ change_date_month_name_char  ($cluster->created_at) }}</td>
            </tr>
            <tr>
                <th width="17%" style="text-align: left">Federation Name:</th>
                <td width="16%" style="text-align: left">{{ $fed_profile[0]->name_of_federation }}</td>
                <td width="16%" style="text-align: left"> State:</td>
                <td width="16%" style="text-align: left">{{ $profile[0]->name_of_state }}</td>
                <th width="16%" style="text-align: left"> District:</th>
                <td width="26%" style="text-align: left">{{ $profile[0]->name_of_district }}</td>
            </tr>
            <tr>
                <th width="17%" style="text-align: left"> Block Name:</th>
                <td width="16%" style="text-align: left">{{ $profile[0]->block }}</td>
                <td width="16%" style="text-align: left"> Village Name: </td>
    ...
    <div class="page-header-title" style="margin-top: -50px;">
        ...
        <table class="table table-bordered table-stripped table1" cellspacing="20" style="width: 65%; margin: auto;margin-top:-50px;">
            <tbody>
        ...
        @php
            ...
            if ($grand_total >= 90) {
                $color = 'green';
            } elseif ($grand_total >= 75 && $grand_total <= 89) {
                $color = 'yellow';
            } elseif ($grand_total >= 75 && $grand_total <= 89) {
                $color = 'yellow';
            } elseif ($grand_total >= 60 && $grand_total <= 74) {
                $color = 'lightgrey';
            } elseif ($grand_total <= 59) {
                $color = 'red';
            }
        @endphp
    ...
    <div class="page-header-title">
        <div class="d-inline">
            <h3
                ...
                ClUSTER CHALLENGES & ACTION PLAN
        ...
        <div style=" width:100%;">
            <div class="page-header-title">
                <div class="d-inline">
                    <h3 style="font-family: Arial, Helvetica, sans-serif;text-align:start;font-size:20px;"
                        style="margin:0px 0px">Cluster Name:&nbsp; {{ $profile[0]->name_of_cluster }}
                    </h3>
                </div>
                <div class="d-inline" style="margin-left:700px;margin-top:-80px;">
                    <h3 style="font-family: Arial, Helvetica, sans-serif;text-align:start;font-size:20px;"
                        style="margin:0px 0px">UIN:&nbsp;{{ $cluster->uin }}
                    </h3>
                </div>
            </div>
            ...
            <table style=" border-collapse: collapse;width:100%;">
                <tbody>
                    <tr>
                        <th
                            style="padding: 8px; text-align: left; border-bottom: 1px solid #ddd;border-right:1px solid  #ddd width:4%;">
                        </th>
                        <th style="padding: 8px; text-align: left; border-bottom: 1px solid #ddd width:96%">Cluster
                            challenges</th>
                    </tr>
                    @php $i=1; @endphp
                    @if (!empty($challenges))
    ...
    <br>
    <footer style="margin-left: 400px; width: 100%; font-size: 30px;margin-top: 10px;">
        <b>Overall Rating</b>
        <div
                style="height: 35px; width: 250px; border: 1px solid black; display: inline-block; margin-top:10px;margin-left:200px;background-color:{{ $color }};text-align:center;">
