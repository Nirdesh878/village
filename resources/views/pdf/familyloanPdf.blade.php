...
<body class="antialiased container mt-5">
    ...
    <div class="page-header-title">
        <div class="d-inline">
            <h2 style="font-family: Arial, Helvetica, sans-serif;text-align:center;font-size:30px;"><u>Family Loan({{$family->uin}})</u>
            </h2>
        </div>
    ...
    <table class="table table-bordered table-stripped table1 " cellspacing="0">
        ...
        <tbody>
            ...
            <tr >
                ...
                <td>{{checkna($business_investment_plan[0]->type_of_business)}}</td>
                <td>{{($loan_repayment[0]->tenure_mode == 1 ? 'Yearly' : ($loan_repayment[0]->tenure_mode == 0 ? 'Monthly' : 'N/A'))}}</td>
                <td>{{checkna($loan_duration)}}</td>
                <td>{{checkna($loan_tenure)}}</td>
            </tr>
        </tbody>
    </table>
    <br>
    <table class="table table-bordered table-stripped table1 " cellspacing="0">
        ...
        <tbody>
            ...
            <tr >
                <td>{{checkna($loan_approvel[0]->get_verified)}}</td>
                <td>{{checkna($loan_approvel[0]->uloan_amount)}}</td>
                <td>{{checkna($loan_approvel[0]->uloan_purpose)}}</td>
                <td>{{checkna($loan_approvel[0]->urepayment_mode)}}</td>
                @php
                $uduration = 'N/A';
            ...
            @endphp
            <td>{{$uduration}}</td>
                <td>{{checkna($loan_approvel[0]->uloan_installments)}}</td>
            </tr>
        </tbody>
    </table>
    <br>
    <table class="table table-bordered table-stripped table1 " cellspacing="0">
        ...
        <tbody>
            <tr>
                ...
                <td>{{ ( !empty($manager[0]->name) ? $manager[0]->name : 'N/A') }}</td>
                <th style="text-align: left;">Date</th>
                <td>{{$loan_approvel[0]->date !='' ? change_date_month_name_char(str_replace('/','-',$loan_approvel[0]->date)) : 'N/A'}}</td>
            </tr>
    ...
    <table class="table table-bordered table-stripped table1 page-break" cellspacing="0">
        ...
        <tbody>
            ...
            <tr >
                <td>{{checkna($loan_disbursement[0]->get_loan)}}</td>
                <td>{{$loan_disbursement[0]->disbursement_date !='' ? change_date_month_name_char(str_replace('/','-',$loan_disbursement[0]->disbursement_date)) : 'N/A'}}</td>
                <td>{{checkna($loan_disbursement[0]->loan_amount)}}</td>
                <td>{{checkna($loan_disbursement[0]->loan_purpose)}}</td>
                <td>{{checkna($loan_disbursement[0]->repayment_mode)}}</td>
                @php
                $duration = 'N/A';
                ...
                @endphp
                <td>{{$duration}}</td>
                <td>{{checkna($loan_disbursement[0]->no_installments)}}</td>
