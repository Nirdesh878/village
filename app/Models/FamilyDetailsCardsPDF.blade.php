    ...
    <div class="canvas_all_pdf">
        <div class="row ">
            ...
            <table class="table table-bordered table-stripped table1 " cellspacing="0">
                ...
                <tbody>
                    <tr style="text-align: center;background-color: #eeeeee;">
                        ...
                        <td style="border:1px solid white">{{ checkna($family_profile[0]->fp_account) }}</td>
            ...
            <table class="table table-bordered table-stripped table1 " cellspacing="0">
                ...
                @if ($family_profile[0]->gov_liveilhood_program == 'No')
                <tbody>
                    <tr>
                    ...
                    <thead>
                        ...
                        @if (!empty($vi_loan))
                            ...
                            @foreach ($vi_loan as $res)
                                @php
                                    $vi_total = $vi_total + checkZero($res->lo_next_year);
                                    $vi_amount = $vi_amount + checkZero($res->lo_principle_amount);
                                    $vi_overdue = $vi_overdue + checkZero($res->overdue);
                                    $vi_cumulative = $vi_cumulative + checkZero($res->total_paid_interest);
                                    $vi_paid = $vi_paid + checkZero($res->current_year_interest);

                                @endphp
                                <tr>
                                    <td style="text-align:left;">VI loan</td>
                                    <td class="tdc">{{ $res->lo_principle_amount }}</td>
                                    <td class="tdc">{{ $res->current_year_interest }}</td>
                                    <td class="tdc">{{ $res->total_paid_interest }}</td>
                                    <td class="tdc">{{ $res->overdue }}</td>
                        ...
                        <tr style="font-weight:bold">
                            ...
                            <th class="tdc" style="background-color:#c2b5b5;">
                                {{ $other_amount + $fed_amount + $cluster_amount + $bank_amount + $money_amount + $shg_amount + $vi_amount }}
                            </th>

                            <th class="tdc" style="background-color:#c2b5b5;">
                                {{ $other_paid + $fed_paid + $cluster_paid + $bank_paid + $money_paid + $shg_paid + $vi_paid }}
                            </th>
                            <th class="tdc" style="background-color:#c2b5b5;">
                                {{ $other_cumulative + $fed_cumulative + $cluster_cumulative + $bank_cumulative + $money_cumulative + $shg_cumulative + $vi_cumulative }}
                            </th>
                            <th class="tdc" style="background-color:#c2b5b5;">
                                {{ $other_overdue + $fed_overdue + $cluster_overdue + $bank_overdue + $money_overdue + $shg_overdue + $vi_overdue }}
                            </th>
                            <th class="tdc" style="background-color:#c2b5b5;">
                                {{ $other_total + $fed_total + $cluster_total + $bank_total + $money_total + $shg_total + $vi_total }}
                            </th>
                        </tr>
                    </thead>
                </table>
                ...
                <tbody >
                    ...
                    <tr>
                        ...
                        <td>{{ $observation_this_year[0]->fdip_observation_agreement }}
                            @if ($observation_this_year[0]->fdip_observation_agreement == 'No')
                            <ol>
