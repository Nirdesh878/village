...
class ClusterAnalysis implements WithHeadings, ShouldAutoSize, WithEvents, WithTitle, FromCollection, WithColumnFormatting, WithMapping
{
    ...
    public function map($res): array
    {
        ...
        (string)$res['Cluster_Book_updation'],
        (string)$res['External_audit_completed'],
        (string)$res['Percentage_of_Defunct'],

        (string)$res['Representation_of_Poorest'],
        (string)$res['poor_current_leadership'],
        (string)$res['Percentage_of_poorest_benefiting_from_all_loans'],


        (string)$res['Cluster_is_covering_its'],
        (string)$res['Time_taken_to_disburse'],
        (string)$res['time_taken_to_approve_loan'],

        (string)$res['Repayment_rate_of_cluster_loan'],
        (string)$res['Cluster_loan_PAR'],
        (string)$res['Percentage_members_assisted_more_than_one'],
        (string)$res['Percentage_Livelihood_purposes'],

        (string)$res['Percentage_of_the_cluster'],
    ...
    public function headings(): array
    {
        return [
            ...
            'Cluster book updation including minute book',
            'External audit completed',
            'Percentage of defunct SHGs in the cluster',

            'Representation of poorest, vulnerable & poor category in the baord members',
            '% of Poor and poorest in leadership positions ',
            'Percentage of poorest, vulnerable & poor families benefitting from all loans',

            'Cluster is covering its operational cost through its own income',
            ...
            'Repayment rate of Cluster loan',
            'PAR status of all loans ( cluster and external)',
            '% of members received more than one loan during last 3 years',
            '% of productive loans (out  of total loans)',

            'Percentage of the cluster members having voluntary savings',
            'No of members contribute to LSF',
    ...
    function cluster_analysis($clus_id){
        ...
        if ($x2 != '') {
            ...
            $data['Repayment_rate_of_cluster_loan'] = (($totalMonths <= 12 ? 10 : ($totalMonths <=24 ? 5 : ($totalMonths > 48 ? 0 : 0))));
