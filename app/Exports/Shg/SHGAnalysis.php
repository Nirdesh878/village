...
class SHGAnalysis implements WithHeadings, ShouldAutoSize, WithEvents, WithTitle, FromCollection, WithColumnFormatting, WithMapping
{
    public $curdate = '';
    public $counter = 1;
    ...
    public function map($res): array
    {
        ...
        return [
            ...
            $res['federation_name'],
            $res['village'],
            (string) $res['Average_participation_of'],
            (string) $res['shg_book_updation'],
            (string) $res['shg_grading_status'],
            (string) $res['shg_percent_of_poorest_internal'],
            (string) $res['shg_percent_of_poorest_other'],
            (string) $res['no_of_leadership_poor'],
            (string) $res['shg_operational_cost'],
            (string) $res['shg_time_taken_loan_disburse'],
            (string) $res['shg_repayment_internal'],
            (string) $res['shg_repayment_other'],
            (string) $res['shg_PAR_status_internal_loan'],
            (string) $res['shg_PAR_status_other_loan'],
            (string) $res['shg_regularity_savings'],
            (string) $res['shg_compulsory_savings'],
            (string) $res['shg_voluntary_savings'],
    ...
    public function headings(): array
    {
        return [
            ...
            [
                ...
                'Federation Name',
                'Name of Village',
                'AVG participation of members during last 12 months',
                'SHG book updation including minute book',
                'Grading status of the SHG during last one year',
                'Percentage of poorest, vulnerable & poor families benefitting from internal lending of the SHG other bank loans',
                'Percentage of poorest, vulnerable & poor families benefitting from other loans',
                'Number of poor/vulnerable and poor families in leadership position',
                'SHG is covering its operational cost through its own income',
                'Time taken to disburse loan to the member hand from approval',
                'Repayment rate of Internal lending',
                'Repayment rate of consolidated external loans',
                'PAR status for different loan - Internal Loan',
                'PAR status for different loan - External Loans',
                'Savings Regularity. All the memebrs saved last 12 months',
                'Compulsory savings increasing trend',
                'Voluntary Savings Trend'
            ]
        ];
    }
