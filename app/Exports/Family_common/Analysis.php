...
class Analysis implements WithHeadings, ShouldAutoSize, WithEvents, WithTitle, FromCollection, WithColumnFormatting, WithMapping
{
    ...
    function family_analysis($family_id)
    {
        ...
        $WealthData = getMstCommonData(7,$family_info->Wealth_Rank);
        $wealthName = $WealthData->isNotEmpty() ? $WealthData[0]->common_values : 'N/A';
        $data['Wealth_Rank'] = $wealthName;
        // prd($data['Wealth_Rank']);
        ...
        $WealthData1 = DB::select($query)[0]->fp_wealth_rank;
        $WealthName1 = getMstCommonData(7, $WealthData1);
        $wealth_rank = $WealthName1->isNotEmpty() ? $WealthName1[0]->common_values : 'N/A';
        // prd($wealth_rank);
